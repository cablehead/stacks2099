...
const BIN = process.env.STACKS2099_BIN || join(here, "..", "target", "debug", "stacks2099");
...
export async function spawnApp() {
  ...
  const proc = spawn(BIN, ["--dev", `127.0.0.1:${port}`, "--store", store], { stdio: "ignore" });
  for (let i = 0; ; i++) {
    try { if ((await fetch(`${base}/`)).ok) break; } catch { /* not up yet */ }
    if (i >= 120) { proc.kill("SIGKILL"); throw new Error("stacks2099 did not start"); }
...
export function launchBrowser() {
  return chromium.launch({ executablePath: CHROMIUM, args: ["--no-sandbox", "--disable-dev-shm-usage"] });
}
