...
**Live terminal, server-rendered.** The terminal renders as an HTML grid and repaints a row at a time, so only what changed redraws. Links are clickable, even when a URL wraps across lines. Wide characters line up. Apps that clear the screen redraw cleanly.
